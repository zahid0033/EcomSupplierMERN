...
class CreateAdmin extends Component {
  ...
  componentDidMount() {
    this.fetchRoles()
    if (this.props.edit) {
      let { name, email, password, roleId, address, phone, status } =
  ...
  sendSave = async () => {
    if (this.state.name === '') {
      ...
      if (!this.props.edit) {
        ...
        await axios
          .post(`/api/admin/update/${this.props.singleData.id}`, dataPost, {
            headers: {
              'Content-Type': 'multipart/form-data',
              'x-access-token': authHeader(),
            },
        ...
        <div className='form-row justify-content-center'>
          ...
          <div className='form-group col-md-6'>
            ...
            <select
              ...
              className='form-control'
              onChange={(value) =>
                this.setState({ role_id: value.target.value })
              }
            >
              {/*<option selected>Choose...</option>*/}
              {this.props.edit ? (
                <option value={this.props.singleData.roleId}>
                  {this.props.singleData.role.role}
                </option>
              ) : (
        ...
        {this.props.edit ? (
          <div className='form-row'>
            <div className='form-group col-md-6'>
              ...
              >
                <option value={this.props.singleData.status}>
                  {this.props.singleData.status}
                </option>
