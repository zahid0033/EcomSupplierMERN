...
class ManageProducts extends Component {
  ...
  exclusiveHandle = async (status, id) => {
    ...
    await axios
      .post(
        `/api/product/exclusiveUpdate`,
        { status: this.state.exclusive, id: id },
  ...
  featuredHandle = async (status, id) => {
    ...
    await axios
      .post(
        `/api/product/featuredUpdate`,
        { status: this.state.featured, id: id },
        { headers: { 'x-access-token': authHeader() } }
      )
      .then((res) => {
    ...
    return this.state.products.map((product, key) => {
      return (
        <tr key={key}>
          ...
          <td>{product.price}</td>
          <td>
            {product.exclusive === 'no' ? (
              <span
                className='btn btn-danger'
                onClick={() =>
                  this.exclusiveHandle(product.exclusive, product.id)
                }
              >
          ...
          <td>
            {product.featured === 'no' ? (
              <span
                className='btn btn-danger'
                onClick={() =>
                  this.featuredHandle(product.featured, product.id)
                }
              >
          ...
          <td>
            <Link
              ...
              to={'/admin/viewProductDetails/' + product.id}
            >
              View
            </Link>
            <span
          ...
          <Card>
            <CardBody>
              <Table responsive>
                ...
                <tbody>{this.fillProductsData()}</tbody>
              </Table>
            </CardBody>
          </Card>
        </div>
